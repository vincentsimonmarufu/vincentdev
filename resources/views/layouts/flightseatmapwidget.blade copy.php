<style>    
    ...
    color: red;
    font-weight: bold;
    grid-column: 1 / -1; /* This spans the entire row */
    padding: 5px 0;
    }
...
<style>
      ...
      display: block;
                ...
                <div class="container" id=""> 
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            <h4 class="mt-2" style="margin-left: 10px;">Flight({{ $seatMapData[0]['carrierCode'] }}{{ $seatMapData[0]['number'] }}) seats detail</h4>
                            <div class="container mt-2">
                                ...
                                @for($i = 0; $i < $metaCount; $i++)    
                                        ...
                                        <div id="container-{{$i+1}}" class="container-box {{ $newactive }}" style="overflow: auto;">
                                            @if($i == 0)                                     
                                                <div class="mb-1"> Select seats for flight </div>
                                            @endif  
                                            @if($i>=1)                                     
                                                <div class="mb-1"> Select seats for <b>{{ $i }}</b> connecting flight </div>
                                            @endif                                              
                                            @foreach($seatMapData[$i]['decks'] as $deck)
                                                <div class="deck-container" style="width: {{ $deck['deckConfiguration']['width'] * 40 }}px;">                                                   
                                                        @foreach ($deck['seats'] as $seat)
                                                            ...
                                                            <div class="seat 
                                                                {{ $status === 'blocked' ? 'blocked-seat' : ($status === 'available' ? 'available-seat' : ($status === 'occupied' ? 'occupied-seat' : '')) }}" 
                                                                data-seat-number="{{ $seat['number'] }}"
                                                                style="grid-column: {{ $seat['coordinates']['y'] + 1 }}; 
                                                                        grid-row: {{ $seat['coordinates']['x'] + 1 }};">
                                                                {{ $seat['number'] }}
                                                            </div>
                                                        @endforeach                                                      
                            ...
                            <div class="container mt-1 mb-2">
                                ...
                                <button id="next-btn" class="btn btn-primary mt-0" data-metacount="{{ $metaCount }}">Next</button>
                            </div>                                                                      
                        </div>                   
                        <div class="col-md-5" style="margin-left: 5%;margin-top: 5%;">                            
                                ...
                                <div class="booking-seats-window">
                                            ...
                                            <div id="selected_seat_info"></div>

                                            <form action="{{ route('flight-price') }}" id="form{{ $flightOfferDetail->id }}" method="GET" class="form">
                                                @csrf 
    ...
    $(document).ready(function() {       
        ...
        $('.deck-container').on('click', '.seat.available-seat', function() {
            ...
            $('#no_of_selected_seats').text('Selected Seats: ' + (selectedSeats.length > 0 ? selectedSeats.join(', ') : 'None'));          
