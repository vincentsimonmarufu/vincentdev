...
class RideService
{
    ...
    {
        return Route::whereHas('buslocs', function (Builder $query) use ($startLocationId) {
            $query->where('id', $startLocationId);
        })->whereHas('buslocs', function (Builder $query) use ($startLocationId, $endLocationId) {
            $query->where('id', $endLocationId)
                ->where('order', '>', function (QueryBuilder $query) use ($startLocationId) {
                    $query->select('order')
                        ->from('location_route')
                        ->where('busloc_id', $startLocationId)
