            ...
            <div class="row align-items-center justify-content-between">
              <div class="col-lg-8">
                <div class="detail-maintitle-in">
                  <h2 class="mb-1">{{ $bus->make }}</h2>
                  <p><i class="fa fa-map-marker-alt me-2"></i>{{ $bus->model }} {{ $bus->year }}</p>
                  <p><i class="fa fa-map-marker-alt me-2"></i>{{ $bus->seater }} seater</p>

                </div>
              </div>
              <div class="col-lg-4">
                <div class="entry-price text-lg-end text-start">
                  <h3 class="mb-0"><span class="d-block theme fs-5 fw-normal">Start From</span> ${{number_format($bus->price,2)}} </h3>/km
                </div>
              </div>
            </div>
          </div>
                ...
                <div class="sidebar-item mb-4 box-shadow p-4">
                  ...
                  <div class="author-news-content d-flex align-items-center">
                    <div class="author-content w-75 ps-4">
                      <h4 class="title mb-1"><a href="#">{{ $bus->user->name . ' '. $bus->user->surname}}</a></h4>
                      <!-- <p class="mb-1">{{ $bus->user->phone }}<br>{{ $bus->user->email }}</p>-->
                    </div>
                  </div>
                </div>
                @foreach ($bus->pictures as $picture)
                <img src="{{ asset('storage/Bus/' . $picture->path) }}" class="img-fluid" alt="bus picture">
                @endforeach
            ...
            <div class="list-sidebar">
              ...
              <div class="sidebar-item mb-4 box-shadow p-4 text-centerb">
                ...
                <ul>
                  ...
                  <li><strong>Address</strong>: {{ $bus->address }}</li>
                  <li><strong>Location</strong>: {{ $bus->city }}, {{ $bus->country }}</li>
                  <li><strong>Price</strong>: $<span id="price">{{ $bus->price }}</span> / km</li>
                  <li><strong>Seater</strong>: <span id="price">{{ $bus->seater }}</span> </li>
                  <li><strong>Specs & Utilities</strong>:
                    <ul>
                      <li>Transmission: {{ $bus->transmission }}</li>
                      <li>Fuel Type: {{ $bus->fuel_type }}</li>
                    </ul>
                  </li>
                </ul>

                <p class="text-info">Book now for ${{$bus->price}}/km</p>
                <p class="text-center">
                  @if ($errors->any())
