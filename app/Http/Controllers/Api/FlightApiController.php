...
use App\Http\Resources\AmadeusFlightResource;
use App\Http\Resources\ResponseResource;
use App\Http\Resources\AmadeusResponseResource;
...
class FlightApiController extends ResponseController
{

  /**
   * Add/reply API for Amadeus flight booking request
   * 
   */
  public function sendReplynew(Request $request)  //  //public function store()     
  {    
    try {     
        $input = $request->all();
        ...
        //   $request->file('image')->storeAs('public/responses/', $image_name_to_store);
        // } else {
        //   $image_name_to_store = "";
        // }
        ...
        if (!empty($user)) {           
          return response()->json(['data' => new AmadeusResponseResource($user)], 200);
        } else {        
          return response()->json(['data' => 'not submitted'], 404);
        }
      } catch (Exception $th) {     
  ...
  /**
   * 
   * Show API for Amadeus flight booking request
   * 
   * @Param flight_booking_id
   * 
   */
  public function myFlightReqtsShowNew($ref){
    try{

      $flightBooking = FlightBooking::where('reference', $ref)->first();
      ...
      $passengers = Passenger::where('flight_booking_id', $flightBooking->id)->get();

      //return $passengers;
     
      $responses = FlightBookingResponse::where('flight_booking_id',  $flightBooking->id)->orderBy('created_at','Desc')->get(); // 'passengers' => $passengers,

      return response()->json(['data' => ['flightRequest' => new AmadeusFlightResource($flightBooking), 'passengers' => PassengerResource::collection($passengers), 'responses' => AmadeusResponseResource::collection($responses)]], 200);
    }catch (Exception $th) {
      return response()->json(['message' => 'An error occurred'], 500);
    }
  ...
  /**
   * 
   * List API for Amadeus flight booking request
   */
  public function myFlightReqtsNew()
  {
    try {
      $flightReq = FlightBooking::orderBy('id', 'Desc')->where('user_id', Auth::user()->id)->get();
      $flightreqts = AmadeusFlightResource::collection($flightReq);      
      return response()->json(['data' =>  $flightreqts], 200);
    } catch (Exception $th) {
      return response()->json(['message' => 'An error occurred'], 500);
    }
  }
  ...
  /**
   * AMdeus Passengers API
   * 
   */
  public function addFlightPassengers(Request $request){
    ...
    //     }
    // } else {
    //     return response()->json(['message' => 'No passengers data provided'], 400);
    // }  
  ...
  /**
   * Add APi for Amadeus flight booking request with passengers
   *
   * @param  \Illuminate\Http\Request  $request
   * @return \Illuminate\Http\Response
   */
  public function amadeusFlightBooking(StoreFlight $request)
  {
    
    try {
          $flightbooking = new FlightBooking;
          $flightbooking->flight_id = $request->flight_id;
          $flightbooking->reference = $request->reference;
          $flightbooking->queuingOfficeId = $request->queuingOfficeId;       
          ...
          $flightbooking->carrierCode = $request->carrierCode;
          $flightbooking->travel_class = $request->travel_class;
          $flightbooking->flight_option = $request->flight_option;
          $flightbooking->status = $request->status; 
          $flightbooking->user_id = $request->user_id;
          $flightbooking->save();  
          ...
          $passengers = $request->input('passengers', []);
                ...
                foreach ($passengers as $index => $passengerData) { 
                    ...
                    $passenger->name = $passengerData['name']['firstName'];
                    $passenger->surname = $passengerData['name']['lastName'];                                     
                    $passenger->dob = $passengerData['dateOfBirth'];
                    $passenger->gender = $passengerData['gender'];                    
                    $passenger->email = $passengerData['contact']['emailAddress'];                   
                    $passenger->code = $passengerData['contact']['phones'][0]['countryCallingCode'];                   
                    $passenger->phone = $passengerData['contact']['phones'][0]['number'];
                    //return $passengerData['contact']['phones'][0]['number']; 
            ...
            if (!empty($flightbooking)) {            
                return response()->json(['success' => true, 'message' => 'Flight request submitted.'], 201);
            } else {
                return response()->json(['success' => false, 'message' => 'Not submitted.'], 404);
            }         
  ...
  // flight request api for new user
  public function store(Request $request)
  {
    ...
    try {
      ...
      $flightRequest->from = $request->from;
      $flightRequest->to = $request->to;
      $flightRequest->airline = $request->airline;
      $flightRequest->departure_date = $request->departure_date;
      $flightRequest->return_date = $request->return_date;
      $flightRequest->travel_class = $request->travel_class;
      $flightRequest->trip_option = $request->trip_option;
      $flightRequest->message = $request->message;
      $flightRequest->user_id = $user->id;
      $flightRequest->save();

      // checking date
      // $startDate1 = $request->departure_date;
      // $endDate1 = $request->return_date;
      ...
      DB::commit();
      return response()->json(['success' => true, 'message' => 'Flight request successfully submitted.'], 201);

      //   //if ($startDate >= $date && $endDate > $startDate) {
      //   if ($startDate >= $date && $endDate >= $date) {
      //     // Start date is greater than or equal to today's date
      //     // End date is greater than start date        
      //     DB::commit();
      //     return response()->json(['success' => true, 'message' => 'Flight request successfully submitted.'], 201);
      //   }
      //   if ($startDate < $date) {
      //     // Start date is less than today's date
      //     // Invalid booking: Start date should be greater than or equal to today's date        
      //     return response()->json(['success' => false, 'message' => 'Departure date should be greater or equal to flight request day'], 200);
      //   }
      //   if ($endDate < $startDate) {
      //     // End date is not greater than start date
      //     // Invalid booking: End date should be greater than start date
      //     return response()->json(['success' => false, 'message' => 'Return date should be greater or equal departure date. '], 200);
      //   }
      //   if ($endDate > $startDate) {
      //     // End date is not greater than start date
      //     // Invalid booking: End date should be greater than start date
      //     DB::commit();
      //     return response()->json(['success' => true, 'message' => 'Flight request successfully submitted.'], 200);
      //   }
    } catch (Exception $th) {
      DB::rollback();
   ...
   * 
   *  */
  public function myFlightReqts()
  {
    try {
      $flightReq = FlightRequest::orderBy('id', 'Desc')->where('user_id', Auth::user()->id)->get();
      $flightreqts = FlightResource::collection($flightReq);
      return response()->json(['success' => true, 'data' => $flightreqts], 200);
    } catch (Exception $th) {
      return response()->json(['success' => false, 'message' => $th->getMessage()], 500);
    }
  }
  ...
  /**
   * flight request detail* using flightrequest_id
   *
   * @param  int  $id
   * @return \Illuminate\Http\Response
   */
  public function show($id)
  {
    try {
      ...
      $flightDetail = new FlightResource($flightRequest);
      $responses = Response::where('flightrequest_id',  $flightRequest->id)->orderBy('created_at', 'Desc')->get();
      $data = ['flightRequest' =>  [$flightDetail], 'flightResponse' => ResponseResource::collection($responses)];
   ...
   * 
   */
  public function sendReply(Request $request)  //  //public function store()     
  {
    try {
      // $request->validate([
      //   'reply' => 'required',
      ...
      if (!empty($user)) {
        //return $this->sendResponse($user, 'Flight response');
        //return response()->json(['success' => true, 'data' => $user], 200);
        return response()->json(['success' => true, 'data' => new ResponseResource($user)], 200);
      } else {
        //return $this->sendError('Something error in flight response');
        return response()->json(['success' => false, 'data' => 'Response not submitted'], 200);
      }
    } catch (Exception $th) {
      return response()->json(['success' => false, 'message' => $th->getMessage()], 500);
    }
   ...
   * flight response
   */
  public function flightResponse($id)
  {
    try {
      $flightRequest = FlightRequest::with('user')->findOrFail($id);
      $responses = Response::where('flightrequest_id',  $flightRequest->id)->orderBy('created_at', 'Desc')->get();
      return $this->sendResponse($responses, 'Flight response');
    } catch (\Exception $th) {
  ...
  {
    try {
      $flights = FlightRequest::orderby('created_at', 'DESC')->get();
      $flightList = new FlightResource($flights);
      return response()->json(['success' => true, 'data' => $flightList], 200);
    } catch (Exception $th) {
      return response()->json(['success' => false, 'message' => $th->getMessage()], 500);
    }
  }
  ...
  public function update(Request $request, $id)
  {
    ...
    try {
      ...
      $flightRequest->save();
      if ($flightRequest) {
        return $this->sendResponse('', 'Flight updated');
      } else {
   ...
   * 
   */
  public function airportListAPI()
  {
    ...
    if ($response_array) {
      return response()->json(['success' => true, 'data' => $response_array, 'message' => 'Airport list'], 200);
    } else {
  ...
  public function airlinesList()
  {
    ...
    // Decode JSON data into PHP array
    $responseArray = json_decode($airlines, true);
    if (!empty($responseArray)) {
      return response()->json(['success' => true, 'data' => $responseArray, 'message' => 'Airlines list'], 200);
    } else {
      return response()->json(['success' => false, 'message' => 'No data'], 200);
    }
  }
